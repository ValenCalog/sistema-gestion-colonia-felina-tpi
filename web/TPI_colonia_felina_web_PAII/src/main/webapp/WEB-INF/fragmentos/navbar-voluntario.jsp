...
<%
    // recuperamos el usuario logueado para sacar su inicial:
    Usuario uLogueado = (Usuario) session.getAttribute("usuarioLogueado");
    String inicialLogueado = "V"; // default "V" de Voluntario
...
<header class="sticky top-0 z-50 flex items-center justify-between border-b border-border-light dark:border-border-dark bg-white/90 dark:bg-surface-dark/90 backdrop-blur-md px-4 md:px-10 py-3 transition-all duration-300">
    ...
    <div class="hidden md:flex flex-1 justify-center">
        <nav class="flex gap-1 bg-gray-100/50 dark:bg-white/5 p-1 rounded-full border border-gray-200 dark:border-white/10">
            <a class="px-5 py-1.5 rounded-full bg-white dark:bg-surface-cardDark text-sm font-bold text-primary shadow-sm transition-all" href="VoluntarioServlet">Inicio</a>
            <a class="px-5 py-1.5 rounded-full text-sm font-medium text-ink-light dark:text-gray-400 hover:text-primary hover:bg-white/50 dark:hover:bg-white/5 transition-all" href="GatoServlet">Gatos</a>
            <a class="px-5 py-1.5 rounded-full text-sm font-medium text-ink-light dark:text-gray-400 hover:text-primary hover:bg-white/50 dark:hover:bg-white/5 transition-all" href="TareaServlet">Tareas</a>
            <a class="px-5 py-1.5 rounded-full text-sm font-medium text-ink-light dark:text-gray-400 hover:text-primary hover:bg-white/50 dark:hover:bg-white/5 transition-all" href="PostulacionServlet">Adopciones</a>
        </nav>
    </div>
